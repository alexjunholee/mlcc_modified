...
class Calibration
{
  public:
    ...
    // LiDAR Settings
    std::string lidar_number_, image_number_;
    std::string data_path_, color_cloud_path_;
    ...
    Calibration(const std::string &CamCfgPaths, const std::string &CalibCfgFile, bool use_ada_voxel)
    {
        ...
        else
        {
            ...
            ROS_INFO_STREAM("Successfully Initialized Voxel Map (Size: " << voxel_map.size() << ")");
            LiDAREdgeExtraction(voxel_map, ransac_dis_threshold_, plane_size_threshold_, lidar_edge_cloud_);
        }
        if (lidar_edge_cloud_->size() == 0)
        {
            ROS_ERROR_STREAM("No Lidar Edges Extracted!");
            exit(-1);
        }
        ROS_INFO_STREAM("Extracted Lidar Edges (Size: " << lidar_edge_cloud_->size() << ")");

        // #ifdef FISHEYE
        // cv::fisheye::estimateNewCameraMatrixForUndistortRectify(camera_.camera_matrix_, camera_.dist_coeffs_,
        // camera_.rgb_img_.size(),
        ...
        ROS_INFO_STREAM("Successfully Loaded All Data!");
    ...
    void loadCameraConfig(const std::string &CamCfgPaths)
    {
        ...
        cv::cv2eigen(camera_.init_ext_, init_ext);
        auto init_ext_inv = init_ext.inverse().eval();
        camera_.ext_R = init_ext_inv.block<3, 3>(0, 0);
        camera_.ext_t = init_ext_inv.block<3, 1>(0, 3);
        cv::Mat init_ext_inv_ = camera_.init_ext_.inv();
        camera_.init_ext_ = init_ext_inv_;
        // camera_.ext_R << camera_.init_ext_.at<double>(0, 0), camera_.init_ext_.at<double>(0, 1),
        //     camera_.init_ext_.at<double>(0, 2), camera_.init_ext_.at<double>(1, 0), camera_.init_ext_.at<double>(1,
        //     1), camera_.init_ext_.at<double>(1, 2), camera_.init_ext_.at<double>(2, 0),
        //     camera_.init_ext_.at<double>(2, 1), camera_.init_ext_.at<double>(2, 2);
        // camera_.ext_t << camera_.init_ext_.at<double>(0, 3), camera_.init_ext_.at<double>(1, 3),
    ...
    void loadCalibConfig(const std::string &config_file)
    {
        ...
        fSettings["LiDARName"] >> lidar_number_;
        fSettings["ImageName"] >> image_number_;
        ...
        camera_.base_pose_ = Eigen::Matrix4d::Identity();
        auto xyz = fSettings["Pose.xyz"];
        auto quat = fSettings["Pose.quat"];
        Eigen::Quaterniond q(quat[0], quat[1], quat[2], quat[3]);
        camera_.base_pose_.block<3, 3>(0, 0) = q.normalized().toRotationMatrix();
        camera_.base_pose_.block<3, 1>(0, 3) << xyz[0], xyz[1], xyz[2];
    ...
    void loadImgAndPointcloud()
    {
        ...
        pcl::io::loadPCDFile(data_path_ + lidar_number_ + ".pcd", *lidar_cloud_);
        pcl::transformPointCloud(*lidar_cloud_, *lidar_cloud_, camera_.base_pose_.inverse().cast<float>());

        pcl::PassThrough<pcl::PointXYZI> pass;
        pass.setInputCloud(lidar_cloud_);
        pass.setFilterFieldName("x");
        pass.setFilterLimits(0, 40.0);
        pass.filter(*lidar_cloud_);

        ROS_INFO_STREAM("Sucessfully load Point Cloud");
        camera_.rgb_img_ = cv::imread(data_path_ + image_number_, cv::IMREAD_COLOR);
        ROS_INFO_STREAM("Sucessfully load Image");
    }

    void cut_voxel(unordered_map<VOXEL_LOC, OCTO_TREE_ROOT *> &feat_map, pcl::PointCloud<pcl::PointXYZI> &pl_feat,
    ...
    void mergePlane(std::vector<Plane *> &origin_list, std::vector<Plane *> &merge_list)
    {
        ...
        for (auto iter = origin_list.end() - 1; iter != origin_list.begin(); iter--)
        {
            for (auto iter2 = origin_list.begin(); iter2 != iter; iter2++)
            {
                ...
                double dis1 = fabs((*iter)->normal(0) * (*iter2)->center(0) + (*iter)->normal(1) * (*iter2)->center(1) +
                                   (*iter)->normal(2) * (*iter2)->center(2) + (*iter)->d);
                double dis2 = fabs((*iter2)->normal(0) * (*iter)->center(0) + (*iter2)->normal(1) * (*iter)->center(1) +
        ...
        for (size_t i = 0; i < origin_list.size(); i++)
        {
            ...
            merge_plane->d =
                -(merge_plane->normal(0) * merge_plane->center(0) + merge_plane->normal(1) * merge_plane->center(1) +
    ...
    void initVoxel(const float voxel_size, std::unordered_map<VOXEL_LOC, Voxel *> &voxel_map)
    {
        ...
        for (auto iter = voxel_map.begin(); iter != voxel_map.end(); iter++)
            if (iter->second->cloud->size() > 20)
                downsample_voxel(*(iter->second->cloud), down_sample_size_);
    }

    void LiDAREdgeExtraction(const std::unordered_map<VOXEL_LOC, Voxel *> &voxel_map, const float ransac_dis_thre,
                             const int plane_size_threshold, pcl::PointCloud<pcl::PointXYZI>::Ptr &lidar_edge_clouds_3d)
    {
        // ros::Rate loop(5000);
    ...
    void projectLine(const Plane *plane1, const Plane *plane2, std::vector<Eigen::Vector3d> &line_point)
    {
        ...
        if (c1_to_line / c2_to_line > 8 || c2_to_line / c1_to_line > 8)
            return;
    ...
    void calcLine(const std::vector<SinglePlane> &plane_lists, const double voxel_size, const Eigen::Vector3d origin,
                  std::vector<pcl::PointCloud<pcl::PointXYZI>> &edge_cloud_lists)
    {
        if (plane_lists.size() >= 2 && plane_lists.size() <= plane_max_size_)
        {
            ...
            for (size_t plane_idx1 = 0; plane_idx1 < plane_lists.size() - 1; plane_idx1++)
            {
                for (size_t plane_idx2 = plane_idx1 + 1; plane_idx2 < plane_lists.size(); plane_idx2++)
                {
                    ...
                    if (theta > theta_max_ && theta < theta_min_)
                    {
                        if (plane_lists[plane_idx1].cloud.size() > 0 || plane_lists[plane_idx2].cloud.size() > 0)
                        {
                            float matrix[4][5];
                            matrix[1][1] = a1;
                            matrix[1][2] = b1;
                            ...
                            if (points.size() == 2)
                            {
                                ...
                                for (float inc = 0; inc <= length; inc += 0.01)
                                {
                                    ...
                                    {
                                        float dis1 =
                                            pow(p.x - plane_lists[plane_idx1].cloud.points[pointIdxNKNSearch1[0]].x,
                                                2) +
                                            pow(p.y - plane_lists[plane_idx1].cloud.points[pointIdxNKNSearch1[0]].y,
                                                2) +
                                            pow(p.z - plane_lists[plane_idx1].cloud.points[pointIdxNKNSearch1[0]].z, 2);
                                        float dis2 =
                                            pow(p.x - plane_lists[plane_idx2].cloud.points[pointIdxNKNSearch2[0]].x,
                                                2) +
                                            pow(p.y - plane_lists[plane_idx2].cloud.points[pointIdxNKNSearch2[0]].y,
                                                2) +
    ...
    void buildVPnp(const Camera &cam, const Vector6d &extrinsic_params, const int dis_threshold,
                   const bool show_residual, const pcl::PointCloud<pcl::PointXYZ>::Ptr &cam_edge_clouds_2d,
                   const pcl::PointCloud<pcl::PointXYZI>::Ptr &lidar_edge_clouds_3d, std::vector<VPnPData> &pnp_list)
    {
        pnp_list.clear();
        cv::Mat camera_matrix =
            (cv::Mat_<double>(3, 3) << cam.fx_, cam.s_, cam.cx_, 0.0, cam.fy_, cam.cy_, 0.0, 0.0, 1.0);
        cv::Mat distortion_coeff = (cv::Mat_<double>(1, 5) << cam.k1_, cam.k2_, cam.p1_, cam.p2_, cam.k3_);
        ...
        cv::Mat r_vec = (cv::Mat_<double>(3, 1) << rotation_vector3.angle() * rotation_vector3.axis().transpose()[0],
        ...
        for (size_t i = 0; i < search_cloud->points.size(); i++)
        {
            ...
            kdtree_lidar->nearestKSearch(searchPoint, K, pointIdxNKNSearchLidar, pointNKNSquaredDistanceLidar);
            if (kdtree_cam->nearestKSearch(searchPoint, K, pointIdxNKNSearch, pointNKNSquaredDistance) > 0)
            {
                bool dis_check = true;
                for (int j = 0; j < K; j++)
                {
                    float distance = sqrt(pow(searchPoint.x - tree_cloud_cam->points[pointIdxNKNSearch[j]].x, 2) +
                ...
                if (dis_check)
                {
                    ...
                    if (p_l_2d.x > 0 && p_l_2d.x < cam.width_ && p_l_2d.y > 0 && p_l_2d.y < cam.height_)
                    {
    ...
    void colorCloud(const Vector6d &extrinsic_params, const int density, const Camera &cam, const cv::Mat &rgb_img,
                    const pcl::PointCloud<pcl::PointXYZI>::Ptr &lidar_cloud, std::string save_path = "")
    {
        Eigen::AngleAxisd rotation_vector3;
        rotation_vector3 = Eigen::AngleAxisd(extrinsic_params[0], Eigen::Vector3d::UnitZ()) *
                           Eigen::AngleAxisd(extrinsic_params[1], Eigen::Vector3d::UnitY()) *
        ...
        if (save_path != "")
            pcl::io::savePCDFileASCII(save_path, *color_cloud);
    }
    ...
    cv::Mat getProjectionImg(const Vector6d &extrinsic_params)
    {
        ...
        ROS_INFO_STREAM("Visualizing lidar (Size :" << lidar_cloud->size() << ")");
